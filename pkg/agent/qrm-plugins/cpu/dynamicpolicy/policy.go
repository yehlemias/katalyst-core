...
func (p *DynamicPolicy) removePod(podUID string, podEntries state.PodEntries, writeThrougCheckpoint bool) error {
	...
	if writeThrougCheckpoint {
		p.state.StoreState()
	}
	return nil
}

func (p *DynamicPolicy) removeContainer(podUID, containerName string, writeThrougCheckpoint bool) error {
	podEntries := p.state.GetPodEntries()
