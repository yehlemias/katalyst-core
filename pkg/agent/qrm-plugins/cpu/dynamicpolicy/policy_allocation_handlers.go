...
func (p *DynamicPolicy) sharedCoresAllocationHandler(ctx context.Context,
	...
	case apiconsts.PodAnnotationMemoryEnhancementNumaBindingEnable:
		return p.sharedCoresWithNUMABindingAllocationHandler(ctx, req, writeThroughCheckpoint)
	default:
		return p.sharedCoresWithoutNUMABindingAllocationHandler(ctx, req, writeThroughCheckpoint)
	}
}

func (p *DynamicPolicy) sharedCoresWithoutNUMABindingAllocationHandler(_ context.Context,
	req *pluginapi.ResourceRequest, writeThroughCheckpoint bool,
) (*pluginapi.ResourceAllocationResponse, error) {
	if req == nil {
	...
	if allocationInfo == nil {
		...
		if !shouldRampUp {
			...
			if poolAllocationInfo == nil {
				...
				p.state.SetAllocationInfo(allocationInfo.PodUid, allocationInfo.ContainerName, allocationInfo, writeThroughCheckpoint)
				_, err = p.doAndCheckPutAllocationInfo(allocationInfo, false, writeThroughCheckpoint)
				if err != nil {
					return nil, err
				}
		...
		if util.PodInplaceUpdateResizing(req) {
			...
			p.state.SetAllocationInfo(allocationInfo.PodUid, allocationInfo.ContainerName, allocationInfo, writeThroughCheckpoint)
			_, err := p.doAndCheckPutAllocationInfoPodResizingAware(originAllocationInfo, allocationInfo, false, true, writeThroughCheckpoint)
			if err != nil {
				general.Errorf("pod: %s/%s, container: %s doAndCheckPutAllocationInfoPodResizingAware failed: %q",
					req.PodNamespace, req.PodName, req.ContainerName, err)
				p.state.SetAllocationInfo(originAllocationInfo.PodUid, originAllocationInfo.ContainerName, originAllocationInfo, writeThroughCheckpoint)
				return nil, err
			}
		} else {
			_, err := p.doAndCheckPutAllocationInfo(allocationInfo, true, writeThroughCheckpoint)
			if err != nil {
	...
	if needSet {
		...
		p.state.SetMachineState(updatedMachineState, writeThroughCheckpoint)
	}
...
func (p *DynamicPolicy) dedicatedCoresAllocationHandler(ctx context.Context,
	...
	switch req.Annotations[apiconsts.PodAnnotationMemoryEnhancementNumaBinding] {
	case apiconsts.PodAnnotationMemoryEnhancementNumaBindingEnable:
		return p.dedicatedCoresWithNUMABindingAllocationHandler(ctx, req, writeThroughCheckpoint)
	default:
		return p.dedicatedCoresWithoutNUMABindingAllocationHandler(ctx, req, writeThroughCheckpoint)
	}
}

func (p *DynamicPolicy) dedicatedCoresWithoutNUMABindingAllocationHandler(_ context.Context,
	_ *pluginapi.ResourceRequest, writeThroughCheckpoint bool,
) (*pluginapi.ResourceAllocationResponse, error) {
...
func (p *DynamicPolicy) dedicatedCoresWithNUMABindingAllocationHandler(ctx context.Context,
	...
	if req.ContainerType == pluginapi.ContainerType_SIDECAR {
		return p.allocationSidecarHandler(ctx, req, apiconsts.PodAnnotationQoSLevelDedicatedCores, writeThroughCheckpoint)
	}
	...
	if oldAllocationInfo == nil {
		...
		p.state.Delete(req.PodUid, req.ContainerName, writeThroughCheckpoint)
		podEntries := p.state.GetPodEntries()
	...
	err = p.adjustAllocationEntries(writeThroughCheckpoint)
	if err != nil {
...
func (p *DynamicPolicy) allocationSidecarHandler(_ context.Context,
	req *pluginapi.ResourceRequest, qosLevel string, writeThroughCheckpoint bool,
) (*pluginapi.ResourceAllocationResponse, error) {
	_, reqFloat64, err := util.GetQuantityFromResourceReq(req)
	if err != nil {
		return nil, fmt.Errorf("getReqQuantityFromResourceReq failed with error: %v", err)
...
func (p *DynamicPolicy) sharedCoresWithNUMABindingAllocationHandler(ctx context.Context,
	...
	if req.ContainerType == pluginapi.ContainerType_SIDECAR {
		return p.allocationSidecarHandler(ctx, req, apiconsts.PodAnnotationQoSLevelSharedCores, writeThroughCheckpoint)
	}
	...
	allocationInfo, err := p.allocateSharedNumaBindingCPUs(req, req.Hint, writeThroughCheckpoint)
...
func (p *DynamicPolicy) allocateSharedNumaBindingCPUs(req *pluginapi.ResourceRequest,
	hint *pluginapi.TopologyHint, writeThroughCheckpoint bool,
	...
	if util.PodInplaceUpdateResizing(req) {
		...
		p.state.SetAllocationInfo(allocationInfo.PodUid, allocationInfo.ContainerName, allocationInfo, writeThroughCheckpoint)
		checkedAllocationInfo, err := p.doAndCheckPutAllocationInfoPodResizingAware(originAllocationInfo, allocationInfo, false, true, writeThroughCheckpoint)
		if err != nil {
		...
		p.state.SetAllocationInfo(allocationInfo.PodUid, allocationInfo.ContainerName, allocationInfo, writeThroughCheckpoint)
		checkedAllocationInfo, err := p.doAndCheckPutAllocationInfo(allocationInfo, true, writeThroughCheckpoint)
		if err != nil {
			return nil, fmt.Errorf("doAndCheckPutAllocationInfo failed with error: %v", err)
		}
		return checkedAllocationInfo, nil
...
func (p *DynamicPolicy) putAllocationsAndAdjustAllocationEntries(allocationInfos []*state.AllocationInfo, incrByReq bool, writeThroughCheckpoint bool) error {
	return p.putAllocationsAndAdjustAllocationEntriesResizeAware(nil, allocationInfos, incrByReq, false, writeThroughCheckpoint)
}

func (p *DynamicPolicy) putAllocationsAndAdjustAllocationEntriesResizeAware(originAllocationInfos, allocationInfos []*state.AllocationInfo, incrByReq, podInplaceUpdateResizing, writeThroughCheckpoint bool) error {
	...
	err := p.adjustPoolsAndIsolatedEntries(poolsQuantityMap, isolatedQuantityMap,
		entries, machineState, writeThroughCheckpoint)
	if err != nil {
...
func (p *DynamicPolicy) adjustAllocationEntries(writeThroughCheckpoint bool) error {
	startTime := time.Now()
	...
	err := p.adjustPoolsAndIsolatedEntries(poolsQuantityMap, isolatedQuantityMap, entries, machineState, writeThroughCheckpoint)
	if err != nil {
		return fmt.Errorf("adjustPoolsAndIsolatedEntries failed with error: %v", err)
	}
...
func (p *DynamicPolicy) adjustPoolsAndIsolatedEntries(
	...
	err = p.applyPoolsAndIsolatedInfo(poolsCPUSet, isolatedCPUSet, entries,
		machineState, state.GetSharedBindingNUMAsFromQuantityMap(poolsQuantityMap), writeThroughCheckpoint)
	if err != nil {
...
func (p *DynamicPolicy) applyPoolsAndIsolatedInfo(poolsCPUSet map[string]machine.CPUSet,
	...
	machineState state.NUMANodeMap, sharedBindingNUMAs sets.Int, writeThroughCheckpoint bool,
) error {
...
func (p *DynamicPolicy) doAndCheckPutAllocationInfoPodResizingAware(originAllocationInfo, allocationInfo *state.AllocationInfo, incrByReq, podInplaceUpdateResizing, writeThroughCheckpoint bool) (*state.AllocationInfo, error) {
	if allocationInfo == nil {
	...
	err := p.putAllocationsAndAdjustAllocationEntriesResizeAware([]*state.AllocationInfo{originAllocationInfo}, []*state.AllocationInfo{allocationInfo}, incrByReq, podInplaceUpdateResizing, writeThroughCheckpoint)
	if err != nil {
...
func (p *DynamicPolicy) doAndCheckPutAllocationInfo(allocationInfo *state.AllocationInfo, incrByReq, writeThroughCheckpoint bool) (*state.AllocationInfo, error) {
	return p.doAndCheckPutAllocationInfoPodResizingAware(nil, allocationInfo, incrByReq, false, writeThroughCheckpoint)
}
...
func (p *DynamicPolicy) systemCoresAllocationHandler(ctx context.Context, req *pluginapi.ResourceRequest, writeThroughCheckpoint bool) (*pluginapi.ResourceAllocationResponse, error) {
	if req.ContainerType == pluginapi.ContainerType_SIDECAR {
		return p.allocationSidecarHandler(ctx, req, apiconsts.PodAnnotationQoSLevelSystemCores, writeThroughCheckpoint)
	}
