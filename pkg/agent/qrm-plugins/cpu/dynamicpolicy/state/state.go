...
type writer interface {
	SetMachineState(numaNodeMap NUMANodeMap, writeThrough bool)
	SetNUMAHeadroom(numaHeadroom map[int]float64, writeThrough bool)
	SetPodEntries(podEntries PodEntries, writeThrough bool)
	SetAllocationInfo(podUID string, containerName string, allocationInfo *AllocationInfo, writeThrough bool)
	SetAllowSharedCoresOverlapReclaimedCores(allowSharedCoresOverlapReclaimedCores, writeThrough bool)

	Delete(podUID string, containerName string, writeThrough bool)
	ClearState()
	StoreState() error
}
