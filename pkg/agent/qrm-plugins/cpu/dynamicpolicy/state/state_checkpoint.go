...
func (sc *stateCheckpoint) SetMachineState(numaNodeMap NUMANodeMap, writeThrough bool) {
	sc.Lock()
	defer sc.Unlock()
...
func (sc *stateCheckpoint) SetNUMAHeadroom(m map[int]float64, writeThrough bool) {
	sc.Lock()
	defer sc.Unlock()
...
func (sc *stateCheckpoint) SetAllocationInfo(podUID string, containerName string, allocationInfo *AllocationInfo, writeThrough bool) {
	sc.Lock()
	defer sc.Unlock()
...
func (sc *stateCheckpoint) SetPodEntries(podEntries PodEntries, writeThrough bool) {
	sc.Lock()
	defer sc.Unlock()
...
func (sc *stateCheckpoint) SetAllowSharedCoresOverlapReclaimedCores(allowSharedCoresOverlapReclaimedCores, writeThrough bool) {
	sc.Lock()
	defer sc.Unlock()
...
func (sc *stateCheckpoint) Delete(podUID string, containerName string, writeThrough bool) {
	sc.Lock()
	defer sc.Unlock()
