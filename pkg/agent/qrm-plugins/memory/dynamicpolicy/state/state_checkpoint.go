...
func (sc *stateCheckpoint) SetMachineState(numaNodeResourcesMap NUMANodeResourcesMap, writeThrough bool) {
	sc.Lock()
	defer sc.Unlock()
...
func (sc *stateCheckpoint) SetNUMAHeadroom(numaHeadroom map[int]int64, writeThrough bool) {
	sc.Lock()
	defer sc.Unlock()
...
func (sc *stateCheckpoint) SetAllocationInfo(resourceName v1.ResourceName, podUID, containerName string, allocationInfo *AllocationInfo, writeThrough bool) {
	sc.Lock()
	defer sc.Unlock()
...
func (sc *stateCheckpoint) SetPodResourceEntries(podResourceEntries PodResourceEntries, writeThrough bool) {
	sc.Lock()
	defer sc.Unlock()
...
func (sc *stateCheckpoint) Delete(resourceName v1.ResourceName, podUID, containerName string, writeThrough bool) {
	sc.Lock()
	defer sc.Unlock()
