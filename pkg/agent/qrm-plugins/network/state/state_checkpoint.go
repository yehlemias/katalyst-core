...
func (sc *stateCheckpoint) SetMachineState(nicMap NICMap, writeThrough bool) {
	sc.Lock()
	defer sc.Unlock()
...
func (sc *stateCheckpoint) SetAllocationInfo(podUID, containerName string, allocationInfo *AllocationInfo, writeThrough bool) {
	sc.Lock()
	defer sc.Unlock()
...
func (sc *stateCheckpoint) Delete(podUID, containerName string, writeThrough bool) {
	sc.Lock()
	defer sc.Unlock()
