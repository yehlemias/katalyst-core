...
func NewQoSRegionShare(ci *types.ContainerInfo, conf *config.Configuration, extraConf interface{}, numaID int,
	...
	isCPUAffinity := numaID != commonstate.FakedNUMAID
	r := &QoSRegionShare{
		QoSRegionBase:    NewQoSRegionBase(regionName, ci.OriginOwnerPoolName, configapi.QoSRegionTypeShare, conf, extraConf, isCPUAffinity, metaReader, metaServer, emitter),
...
func (r *QoSRegionShare) updateProvisionPolicy() {
	...
	for _, internal := range r.provisionPolicies {
		...
		internal.policy.SetCPUAffinityNUMAs(r.cpuAffinityNUMAs, r.isCPUAffinity)
